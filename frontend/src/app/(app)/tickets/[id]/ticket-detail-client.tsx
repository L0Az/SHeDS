...
function CommentsSection({ ticketId, role }: { ticketId: number; role: string }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {comments.length === 0 ? (
        <p className="text-sm text-slate-400 italic">{t("ticket_no_comments")}</p>
      ) : (
          ...
          {comments.map((c) => (
            <li
              ...
              className={`rounded-lg border px-4 py-3 text-sm ${
                c.is_private ? "border-amber-200 bg-amber-50" : "border-slate-100 bg-slate-50"
              }`}
            >
                ...
                <div className="flex items-center gap-2">
                  {c.is_private && (
                    <span className="flex items-center gap-1 text-xs text-amber-600">
                      <Lock className="h-3 w-3" /> {t("ticket_private_badge")}
                    </span>
                  )}
      ...
      <div className="flex flex-col gap-2">
        <Textarea
          placeholder={t("ticket_comment_placeholder")}
          rows={3}
          value={body}
...
export function TicketDetailClient({ ticket, departments, categories, assignees, role, userId }: TicketDetailClientProps) {
  ...
  const { register, handleSubmit, control, watch, setValue, formState: { isSubmitting, isDirty } } = useForm<FormData>({
  ...
  return (
    <form onSubmit={handleSubmit(onSave)}>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 flex flex-col gap-4">
          <div className="rounded-xl border border-slate-200 bg-white p-6">
            <h2 className="mb-4 font-semibold text-slate-800">{t("ticket_details")}</h2>
            <div className="flex flex-col gap-4">
              {canManage ? (
                <Input label={t("title")} error={undefined} {...register("title")} />
              ) : (
                <div>
                  <p className="text-xs font-medium text-slate-500 mb-1">{t("title")}</p>
                  <p className="text-slate-900 font-medium">{ticket.title}</p>
                </div>
              )}
              {canManage ? (
                <Textarea label={t("description")} rows={6} {...register("description")} />
              ) : (
                <div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-slate-800">{t("ticket_attachments")}</h2>
            ...
            {attachments.length === 0 ? (
              <p className="text-sm text-slate-500">{t("ticket_no_attachments")}</p>
            ) : (
                ...
                {attachments.map((a) => (
                  <li key={a.id} className="flex items-center justify-between gap-2 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <a href={a.file_url} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-slate-800 truncate hover:underline">
                        {a.original_filename}
                      </a>
        ...
        <aside className="flex flex-col gap-4">
          <div className="rounded-xl border border-slate-200 bg-white p-5">
            <h3 className="mb-4 text-sm font-semibold text-slate-700 uppercase tracking-wide">{t("ticket_properties")}</h3>
            <div className="flex flex-col gap-4">
              {canManage ? (
                <Controller name="status" control={control} render={({ field }) => (
                  <AppSelect label={t("status")} value={field.value} onValueChange={(v) => field.onChange(v ?? "open")} options={statusOptions} />
                )} />
              ) : (
                <div>
              ...
              {canManage ? (
                <Controller name="priority" control={control} render={({ field }) => (
                  <AppSelect label={t("priority")} value={field.value} onValueChange={(v) => field.onChange(v ?? "medium")} options={priorityOptions} />
                )} />
              ) : (
                <div>
                  <p className="text-xs font-medium text-slate-500 mb-1">{t("priority")}</p>
                  <PriorityBadge priority={ticket.priority} />
                </div>
              )}
              ...
              {canManage ? (
                <div className="flex flex-col gap-1">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-slate-700">{t("ticket_assigned_to")}</span>
                    <button
                      type="button"
                      onClick={() => setValue("assigned_to", String(userId), { shouldDirty: true })}
                  ...
                  <Controller name="assigned_to" control={control} render={({ field }) => (
                    <AppSelect
                      ...
                      onValueChange={(v) => field.onChange(v ?? "")}
                      placeholder={t("ticket_unassigned")}
                      options={assignees.map((a) => ({ value: String(a.id), label: a.name || a.email || String(a.id) }))}
                    />
                  )} />
                </div>
              ) : (
                <div>
                  <p className="text-xs font-medium text-slate-500 mb-1">{t("ticket_assigned_to")}</p>
                  <p className="text-slate-700">{ticket.assigned_to_name ?? t("ticket_unassigned")}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5">
            <h3 className="mb-3 text-sm font-semibold text-slate-700 uppercase tracking-wide">{t("ticket_timestamps")}</h3>
            <dl className="flex flex-col gap-2 text-sm">
              ...
              {ticket.closed_at && (
                <div className="flex justify-between">
                  <dt className="text-slate-500">{t("closed")}</dt>
      ...
      <AppDialog
        ...
        title={t("ticket_delete_title")}
        description={t("ticket_delete_desc")}
      >
